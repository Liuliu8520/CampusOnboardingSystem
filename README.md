...
- 学院、专业、班级使用独立字典表维护，学生档案通过外键 `college_id`、`major_id`、`class_id` 关联，避免名称冗余和不一致。
- 学院管理页面支持维护学院→专业→班级三级结构，新增时自动分配排序号。
- 学生管理支持按学院名称、缴费状态、资料核验状态、宿舍分配状态多条件筛选。
- 仪表盘学院报到率按真实外键关联统计，每个学院独立计算报到率。
...
- 学院管理：维护学院、专业、班级三级字典结构，支持新增、编辑、删除，新增时自动分配排序号。
- 学生管理：学生增删改查、多条件筛选（学院名称、缴费状态、核验状态、宿舍状态）、重置密码、确认报到、项目级缴费状态维护。
- 宿舍管理：楼栋管理、批量生成房间、床位占用图、床位学生姓名和学号展示。
- 缴费项目：配置缴费项目、金额、必缴/选缴、启用/停用。
- 资格修改审核：审核学生提交的信息修改申请。
- 公告管理：发布和维护迎新通知。
- 数据看板：展示学院报到率、学生数量、缴费率、报到率、宿舍分配情况和趋势数据。
...
- `admins`：管理员账号
- `students`：学生档案和报到总状态，通过 `college_id` / `major_id` / `class_id` 外键关联学院、专业、班级
- `colleges`：学院字典
- `majors`：专业字典（外键关联学院）
- `classes`：班级字典（外键关联专业）
- `fee_items`：缴费项目
...
验证码为登录页展示的数学公式计算结果（如 `10 - 7 = ?`，输入 `3`），点击题目可刷新。
